...
# ex: 二回行動
# アニメーション開始
    execute if score @s General.Mob.Tick matches 35 as @e[type=item_display,tag=3F.AJ,tag=3F.AJLink,sort=nearest,limit=1] run function animated_java:lexiel/animations/attack2/play

# フィールド設定
    execute if score @s General.Mob.Tick matches 42 run data modify storage api: Argument.FieldOverride set value {Color:16731392,Frames:[20335,20336,20337],Scale:[7f,7f,0.1f],Transformation:{left_rotation:[0.704f, -0.062f, 0.062f, 0.704f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f]}}
    execute if score @s General.Mob.Tick matches 42 run function asset:mob/0123.lexiel_v3/tick/common/slash3

    execute if score @s General.Mob.Tick matches 52 run data modify storage api: Argument.FieldOverride set value {Color:16731392,Frames:[20335,20336,20337],Scale:[7f,7f,0.1f],Transformation:{left_rotation: [0.183f, 0.683f, 0.683f, -0.183f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f]}}
    execute if score @s General.Mob.Tick matches 52 run function asset:mob/0123.lexiel_v3/tick/common/slash3

# リセット
    execute if score @s General.Mob.Tick matches 60 run function asset:mob/0123.lexiel_v3/tick/skill_reset
